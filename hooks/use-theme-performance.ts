  ...
  const measureTogglePerformance = () => {
    ...
    return () => {
      ...
      performanceMetrics.current = {
        avgResponseTime: (performanceMetrics.current.avgResponseTime * performanceMetrics.current.sampleCount + responseTime) / (performanceMetrics.current.sampleCount + 1),
